...
document.addEventListener('DOMContentLoaded', function() {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (file.name.endsWith('.cfg')) {
                ...
                reader.onload = function(e) {
                    ...
                    const messages = wesnothWMLUtils.extractMessages(content, events);
                    newFilesData.push({
                        fileName: file.name,
                        events: events,
                        messages: messages
    ...
    function processPseudoFiles(files) {
        files.forEach(file => {
            ...
            const events = wesnothWMLUtils.extractEvents(file.content);
            const messages = wesnothWMLUtils.extractMessages(file.content, events);
            allFilesData.push({
                fileName: file.name,
                events: events,
                messages: messages
    ...
    function renderEventsCombined() {
        ...
        allFilesData.forEach(file => {
            file.events.forEach(event => {
                allEvents.push({...event, fileName: file.fileName});
            });
        });
        ...
        container.className = 'manager-event-events-container-inner';
        ...
        for (const [groupKey, eventGroup] of Object.entries(groupedEvents)) {
            ...
            eventGroup.forEach((event, index) => {
                ...
                const blockHeader = document.createElement('div');
                blockHeader.className = 'manager-event-event-subheader';
                blockHeader.textContent = `${event.fileName}: ${i18n.t('events_manager.event')} #${index + 1}`;
    ...
    function renderMessagesCombined() {
        ...
        allFilesData.forEach(file => {
            file.messages.forEach(msg => {
                allMessages.push({...msg, fileName: file.fileName});
            });
        });
        ...
        const container = document.createElement('div');
        container.className = 'manager-event-messages-container-inner';
        ...
        for (const [groupKey, messageGroup] of Object.entries(groupedMessages)) {
            ...
            messageGroup.forEach(msg => {
                ...
                speaker.innerHTML = `
                    ...
                    <span class="manager-event-message-file">(${msg.fileName})</span>
                `;
